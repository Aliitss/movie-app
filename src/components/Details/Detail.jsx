...
function Detail() {
  ...
  useEffect(() => {
    ...
    .catch(error => {
      sweetAl (
        <h2>We broke something, come back later 😥</h2>
      )
    })
  }, [movieID]);
    ...
    <>
      ...
      { movieDetail && 
        <>
          ...
          <div className="row">
            <div className="col-4">
              <img src={`https://image.tmdb.org/t/p/w500/${movieDetail.poster_path}`} alt='...' className='img-fluid mb-4 mt-5'/>
            </div>
            <div className="col-8">
              ...
              <ul> 
                {
                  movieDetail.genres.map(oneMovie => <li key={oneMovie.id}>{oneMovie.name}</li>)
                }
              </ul>
            </div>
          </div>
